...
const UserWalletCoupons = () => {
    ...
    const CreatePin = () => {
        ...
        fetch(urls.apiurl + 'coupon/create_wallet_pin', postOptions)
    ...
    const getCiuponCodes = () => {
        setLoading(true)
        fetch(urls.apiurl +'coupon/'+ token+'/wallet_coupons')
            .then((response) => response.json())
            .then((json) => {
                if (json.status == 1) {
    ...
    const searchCouponCode = () => {
        setLoading(true)
        fetch(urls.apiurl +'coupon/'+ token+'/'+ couponCode+'/search_coupon')
            .then((response) => response.json())
            .then((json) => {
                if (json.status == 1) {
                    setTransactions(json.result)
    ...
    const getList =  <div class="row">					
        ...
        <div class="box">
            <div class="box-header with-border">
              <h4 class="box-title">Wallet Coupons</h4>
            </div>
            <div class="box-body pt-0">
                <div class="table-responsive">
                  ...
                  {transactions.map(tr =>
                    <tr>
                      <td>{tr.coupon}</td>
                      <td><a href="#">{NumberToNaira(tr.amount)}</a></td>
...
<button onClick={()=>CreatePin()} className='btn btn-primary'>Generate Wallet Coupon</button>
                            }
...
export default UserWalletCoupons;
