...
class UsersController extends Controller
{
    ...
    public function check_pending_package(Request $request, $login_token){
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
...
/*
        ...
        else
        {
          ...
          if($token_check != 0)
          {
              ...
              if($ifPhoneExists > 1){
               ...
               where username = ?',[$request->first_name, $request->last_name, $request->phone, $user->username]);
  ...
  //return $validator->errors();
        ...
        else
        {
          ...
          where type = ? and status = ? and username = ?',[2, 'Package', 0, $request->username]);
...
//insert new package transaction
          ...
          values (?, ?, ?, ?, ?)', [
            ...
            where username = ?',[$request->package, $package_amount, 0, 0, $request->username]);
...
//                ->Where('status', 0)
    ...
    public function referral_list(Request $request, $login_token, $num){
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
    ...
    public function verify_bank_account(Request $request, $login_token, $account_number, $bank_code)
    {
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
        if($token_check != 0)
