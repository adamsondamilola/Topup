...
if($data == true){
    ...
    where username = ?',[$total_balance, $cashback_balance, $main_wallet, auth()->user()->username]);
    //Insert transaction
...
$getPin = DB::table('users')
        ...
        if(strlen($request->network) < 1 ){
            ...
            return response()->json(['status' => 0, 'message' => 'Quantity should not be more than 50.'], 401);
        }
        else if(!is_numeric($request->amount)){
            return response()->json(['status' => 0, 'message' => 'Amount is invalid.'], 401);
        }
        else if($request->amount > $checkWallet->main_wallet){
            return response()->json(['status' => 0, 'message' => 'Insufficient balance'], 401);
        }
        else if(!is_numeric($request->amount)){
            return response()->json(['status' => 0, 'message' => 'Amount is invalid.'], 401);
        }
...
//return response()->json(['status' => 0, 'message' => $network], 401);
         ...
         else
         {
           ...
           //  echo "The number is: $x <br>";
             ...
             where amount = ? and network = ? and status = ? limit 1 ',[0, auth()->user()->username, $unique_id, $request->amount, $network, 1]);
           }
...
if($data == true){
    ...
    public function get_airtime_details(Request $request, $id){
        $token_check = $this->check_login_token();
        if($token_check != 0)
        {
            $user = DB::table('users')
...
//for whatsapp chatbot
    ...
    public function get_airtime_list(Request $request){
        $token_check = $this->check_login_token();
        if($token_check != 0)
        {
            $user = DB::table('users')
    ...
    public function get_airtime_list_by_network(Request $request, $network){
        $token_check = $this->check_login_token();
        if($token_check != 0)
        {
            $user = DB::table('users')
            ...
            $airtime_voucher = DB::table('airtime_vouchers')
            ->Where('username', $user->username)
            ->Where('network', $network)
            ->orderBy('id', 'desc');

            $airtime_vouchers = $airtime_voucher
            ->take(50)
            ->get();
            ...
            if($airtime_voucher->count() < 1){
                return response()->json(['status' => 0, 'message' => 'No airtime pin found'], 401);
            }
