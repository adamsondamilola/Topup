...
class TransactionsController extends Controller
{
  ...
  public function calculate_commission_and_points($upline_username, $my_username, $my_package)
  {
    ...
    public function transactions(Request $request, $login_token, $num){
    ...
    public function search_transactions(Request $request, $login_token, $search){
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
    ...
    public function view_transactions(Request $request, $login_token, $id){
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
    ...
    public function cashback_transactions(Request $request, $login_token, $num){
        $token_check = $this->check_login_token($login_token);
        $user_id=$token_check;
  ...
  //return $validator->errors();
    ...
    public function verify_transaction(Request $request, $login_token, $transaction_id){
...
//share bonus and activate account
            //  if($user->referral != null && $user->referral != ''){
              ...
              where status = ? and package = ? and username = ?',[1, "ATM Card", 0, $user->package, $user->username]);


              $this->calculate_commission_and_points($user->referral, $user->username, $user->package);
...
//inset
    ...
    public function activate_package_from_wallet(Request $request, $login_token){
    ...
    return response()->json(['status' => 0, 'message' => 'Main wallet balance too low.', 'result' => 'Empty'], 401);
      ...
      //share bonus and activate account
                ...
                where status = ? and package = ? and username = ?',[1, 0, $user->package, $user->username]);
