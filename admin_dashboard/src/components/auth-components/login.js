...
fetch(urls.apiurl +'user/'+ token+'/user_details/')
    ...
    const loginAction = () =>{
        ...
        fetch(urls.apiurl + 'auth/login', postOptions)
                    .then((response) => response.json())
                    .then((json) => {
                        if (json.status == 1 && json.role == "Admin") {
                            setErrMsg(null)
                            setLoading(false)

                            localStorage.setItem('loginToken', json.login_token);
/*
                    fetch(urls.apiurl + 'user/'+json.login_token+'/user_details')
                    .then((response) => response.json())
                    .then((json) => {
                        if (json.status == 1) {
                            setErrMsg(null)
                    ...
                    */
                    ...
                    //setSuccessMsg("Login was successful. However, dashboard is not ready yet.");
