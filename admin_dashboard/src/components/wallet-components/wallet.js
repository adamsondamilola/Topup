...
const Wallet = () => {
    ...
    const [bankName, setBankName] = useState(null);
            ...
            const checkIfMonifyHasPendingTransaction = () => {
              ...
              if(monnifyToken != null){ 
                    ...
                    setLoading(true)
              fetch(urls.apiurl +'monnify/check_pending_transfer/', postOptions)
                  .then((response) => response.json())
                  .then((json) => {
                      if (json.status == 1) { 
                  ...
                  .then((json) => {
                      if (json.status == 1) {   
                       // alert(json.monnify.bank_name)                     
                        setBankName(json.monnify.bank_name)
                        setAccountNumber(json.monnify.account_number)
                        setAccountName(json.monnify.account_name)
    ...
    const getTransactions = async () => {
        setLoading(true)
        const response = await fetch(urls.apiurl +'transaction/'+ token+'/10/transactions');
