...
const [airtelAirtime, setAirtelAirtime] = useState(null);
    ...
    const getDetails = () => {
        setLoading(true)
        fetch(urls.apiurl +'admin/'+ token+'/commission-details/')
    ...
    const updateAction = () =>{
        ...
        fetch(urls.apiurl + 'admin/update-airtime-print-commission/', postOptions)
                    .then((response) => response.json())
                    .then((json) => {
                        if (json.status == 1) {
                            setErrMsg(null)
...
export default AirtimePrintCommissions;
