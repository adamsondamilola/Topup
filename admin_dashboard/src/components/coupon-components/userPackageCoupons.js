...
//                amount: amount,
        ...
        fetch(urls.apiurl + 'coupon/create_package_pin', postOptions)
    ...
    const getCiuponCodes = () => {
        setLoading(true)
        fetch(urls.apiurl +'coupon/'+ token+'/package_coupons')
            .then((response) => response.json())
            .then((json) => {
                if (json.status == 1) {
...
{isLoading? <LoadingImage /> :
<button onClick={()=>CreatePin()} className='btn btn-primary'>Generate Package Coupon</button>
                            }
...
export default UserPackageCoupons;
