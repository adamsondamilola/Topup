...
const ManageAirtime = () => {
    ...
    const searchTransactions = (search) => {
        setLoading(true)
        const headers = { 'Content-Type': 'application/json' }
    fetch(urls.apiurl +'admin/'+ token+'/'+search+'/admin_search_airtime_pins', { headers })
        .then(response => response.json())
        .then(data => {
            if (data.status === 1) {
                setTransactions(data.result)                    
              ...
              <div class="col-xl-12 col-12">
                  ...
                  <div class="box">
                      <div class="box-header with-border">
                        <h4 class="box-title">Manage Airtime <Link to="/admin/add_airtime" className='btn btn-primary'> + </Link> </h4>
                      </div>
