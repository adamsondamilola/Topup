...
const PrintAirtime = () => {
  ...
  <table class="table">
    ...
    <tr>
      ...
      <td class="text-end">{cardDetails_.dial_code}</td>
    </tr>
    <tr>
      <th><a href="javascript:void(0)">S/N</a></th>
      <td class="text-end">{c.sno}</td>
    </tr>
    </tbody>
   </table>
   </div>
...
//                            setSuccessMsg("Fund Successfully Added")
    ...
    const getAirtimePrintDetails = (id) => {
        setLoading(true)
            ...
            .then((json) => {
                if (json.status == 1) {
                   // setCardDetails(json.result.json_data.TXN_EPIN[0])
                   //alert(JSON.parse(JSON.stringify(json.result.json_data)))
//                   let x = JSON.parse(JSON.stringify(json.result.json_data))
                   let x = JSON.parse(json.result.json_data)
                   //alert(x)
                   let y = x.TXN_EPIN
                   setCardDetails(y)
                   setCardDetails_(json.result)
                   //alert(y)
                   //let z = JSON.parse(y.TXN_EPIN)
                   //alert(JSON.parse(z))
