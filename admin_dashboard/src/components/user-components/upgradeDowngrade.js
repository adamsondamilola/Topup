...
const getPackages = () => {
    ...
    const upgradeAction = () => {
        ...
        fetch(urls.apiurl + 'admin/'+ token+'/'+ username+'/upgrade_downgrade', postOptions)
                    .then((response) => response.json())
                    .then((json) => {
                        if (json.status == 1) {
                            setErrMsg(null)
                            setLoading(false)
                            alert(json.message)
                            window.location.replace("/admin/"+username+"/user_details")
                        }
                        else {
                      ...
                      <div class="col-6 mt-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" onChange={e => setNewPackage(e.target.value)}>
                                        ...
                                        {x.package} {userData.country == "Nigeria"?NumberToNaira(x.amount): "$"+x.amtusd}
                                        </option>)}
                                    </select>
                                </div>                                
                            </div>
                            ...
                            <div class="col-6 mt-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" onChange={e => setAction(e.target.value)}>
                                    <option value={""}>Select</option>
                                    <option value={"Activate"}>Activate Package</option>
                                    <option value={"Do Not Activate"}>Do Not Activate</option>                                        
                                    </select>
                                </div>                                
                            </div>
